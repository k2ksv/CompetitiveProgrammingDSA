...
void Solve() {
    ...
    for (int i = 0; i < x; i++) { cin >> v[i].first; v[i].second = 1;}
    for (int i = 0; i < y; i++) { cin >> v[x+i].first; v[x+i].second = 2;}
    sort(v.rbegin(), v.rend());
    ll ans = 0;
    int a = 0, b = 0, tot = 0;
    for (int i = 0; i < x+y && tot < n+m-1; i++) {
        int type = v[i].second;
        if (i+1 < x + y && v[i].first == v[i+1].first) {
            type = 3; 
            i++; 
        }
        if (type == 1 && a == n) continue;
        if (type == 2 && b == m) continue;
        if (type == 1) a++;
        if (type == 2) b++;
        tot++;
        ans += v[i].first;
